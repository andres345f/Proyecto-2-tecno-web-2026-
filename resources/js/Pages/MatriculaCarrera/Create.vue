    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6 max-w-5xl mx-auto w-full">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-extrabold tracking-tight text-foreground">Inscripción / Matrícula de Carrera</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Selecciona un estudiante existente y asígnale su carrera correspondiente.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Sube un archivo de Excel exportado como CSV para crear las cuentas y matricularlas de una vez.</CardDescription>
                    </CardHeader>
                    <CardContent class="space-y-4">
                        <div class="text-xs text-muted-foreground space-y-2 bg-muted/40 p-3 rounded-lg border border-border/50">
                            ...
                            <ul class="list-disc list-inside font-mono text-[11px] text-foreground space-y-1">
                                ...
                                <li><span class="font-bold">codigo_oferta</span>: Código de la carrera (ej. INF-01, DER-02).</li>
                            </ul>
                            <p class="mt-2 text-[11px]">
                                * El sistema creará el usuario, le asignará el rol <span class="font-semibold text-foreground">Estudiante</span>,
                                generará un código de estudiante aleatorio de 9 dígitos y establecerá ese mismo código como su contraseña inicial.
                            </p>
                        ...
                        <form @submit.prevent="submitImport" class="space-y-4">
                            <div class="space-y-2">
                                ...
                                >
                                    <div v-if="!importForm.archivo" class="space-y-2">
                                        ...
                                        <p class="text-sm font-medium text-foreground">Hacer clic para subir el archivo</p>
                                        <p class="text-xs text-muted-foreground">Solo archivos delimitados por comas (.csv)</p>
                                    </div>
                                    <div v-else class="space-y-3">
                                        ...
                                        <Button variant="ghost" size="sm" type="button" @click.stop="clearFile" class="text-destructive hover:bg-destructive/10 h-7 px-2">
                                            Quitar archivo
                                        </Button>
                                    </div>
                                </div>
                            ...
                            <div v-if="$page.props.errors.import_errors && ($page.props.errors.import_errors as any).length > 0" class="space-y-2 mt-4">
                                <Label class="text-destructive font-semibold text-xs">Errores encontrados en el archivo:</Label>
                                <div class="max-h-40 overflow-y-auto bg-destructive/5 border border-destructive/20 rounded-lg p-3 text-xs text-destructive space-y-1 font-mono">
                                    <div v-for="(error, index) in ($page.props.errors.import_errors as any)" :key="index">
                                        • {{ error }}
                                    </div>
                                </div>
                            </div>
